...
let JwtStrategy = class JwtStrategy extends (0, passport_1.PassportStrategy)(passport_jwt_1.Strategy) {
    constructor(config, usersService) {
        ...
        if (!config.get('JWT_ACCESS_TOKEN_SECRET')) {
    ...
    async validate(payload) {
        ...
        if (!user || user.tokenVersion !== payload.tv) {
            throw new common_1.UnauthorizedException('Access token is no longer valid');
        }
        const allowedRoles = (0, role_utils_1.getAssignableRoles)(user);
        if (!allowedRoles.has(payload.role)) {
            throw new common_1.UnauthorizedException('Access token role is no longer permitted');
        }
        return {
            id: user.id,
            email: user.email,
